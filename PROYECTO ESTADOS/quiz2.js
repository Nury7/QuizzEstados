...
function escogerPregunta(n) {
  ...
  select_id("pregunta").innerHTML ='Cual es la capital de: <br>'+ pregunta.nombre
...
function desordenarRespuestas(pregunta) {
  ...
  capitales = estadosList;
   aleatorio = Math.floor(Math.random()*capitales.length);
   aleatorio2 = Math.floor(Math.random()*capitales.length);
   aleatorio3 = Math.floor(Math.random()*capitales.length);


  posibles_respuestas = [
    pregunta.capital,
    capitales[aleatorio].capital,
    capitales[aleatorio2].capital,
    capitales[aleatorio3].capital
...
function oprimir_btn(i) {
  ...
  if (posibles_respuestas[i] == pregunta.capital) {
    preguntas_correctas++;
  ...
  for (let j = 0; j < 4; j++) {
    if (posibles_respuestas[j] == pregunta.capital) {
